...
//SensorTools V1.0 by Tsuyoshi Perogi (hasherbasher@protonmail.com)
//Automated version of SensorTool V1.0 that allows any vehicle to replace their MAF sensor in place of a GM 2 or 3 bar map sensor.
//Adds support for GM MAP sensors 2 + 3 bar. 
...
int sensorValue2 = analogRead(A5);// Raw analog voltage reading from OEM MAF/MAP
float voltage2= sensorValue1 * (5.0 / 1023.0);//Calculations to convert sensor values into valtage
...
//------------GM 3 bar map sensor-------------------------------------------------------------------- 
//float out[] = {X0,X1,X2,X3,X4,X5,X6,X7,X8,X9};//GM 3 bar
//float in [] = {0, 0.619,0.778, 0.937, 1.096, 1.255, 1.414, 1.573, 3.163, 4.754}; //GM 3 bar MAP sensor 
//float val = sensorValue2;
//float SignalOut = FmultiMap(val, in, out, 10);
//float Value = (SignalOut * 51);
//-------------GM 2 bar map sensor--------------------------------------------------------------------
float out[] = {X0,X1,X2,X3,X4,X5,X6,X7,X8,X9,X10};//GM 2 bar
float in [] = {0, 0.273,0.526, 0.779, 1.032, 1.285, 1.538, 1.791, 2.044, 2.297, 4.827}; //GM 2 bar MAP sensor 
float val = sensorValue2;
